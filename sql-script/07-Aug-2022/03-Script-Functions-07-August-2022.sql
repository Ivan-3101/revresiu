 ...
 If(party='Account') Then
			  If(vpatype='Payee') Then
				num =(select count(vld.ilivemessageid) from transactions.vw_livetrans_dashboard_with_joins vld where
					  ...
					  at time zone 'utc' at time zone timeZone <= enddate and vcpayeeaccount = vpaaddress);
			 ...
			 Then
			   Return query  SELECT vld.ilivemessageid, vld.vcmsgid, vld.vcuniquetransid, vld.dttrxntime at time zone 'utc' at time zone timeZone,
                                    ...
                                    payervpa,
                                    payeevpa
                             FROM transactions.vw_livetrans_dashboard_with_joins vld where vld.dttrxntime at time zone 'utc' at time zone timeZone >= startdate and vld.dttrxntime at time zone 'utc' at time zone timeZone <=enddate
    and vcpayeeaccount = vpaaddress ;
Else
...
End If;
	...
	Elseif(vpatype='Payer') Then
	 ...
	 num =(select count(vld.ilivemessageid) from transactions.vw_livetrans_dashboard_with_joins vld where  vld.dttrxntime at time zone 'utc' at
...
Elseif(vpatype='Both') Then
 ...
 num =(select count(vld.ilivemessageid)  FROM transactions.vw_livetrans_dashboard_with_joins vld where vld.dttrxntime at time zone 'utc' at time zone timeZone >= startdate
	 ...
	 if(maxrecord > num )
		...
		Return query SELECT vld.ilivemessageid, vld.vcmsgid, vld.vcuniquetransid, vld.dttrxntime at time zone 'utc' at time zone timeZone,
                            ...
                            payervpa,
                            payeevpa
                     FROM transactions.vw_livetrans_dashboard_with_joins vld where vld.dttrxntime at time zone 'utc' at time zone timeZone >=startdate
...
elseif(party='VPA') then
  If(vpatype='Payee') Then
				num =(select count(vld.ilivemessageid) from transactions.vw_livetrans_dashboard_with_joins vld where
					  vld.dttrxntime at time zone 'utc' at time zone timeZone >= startdate and vld.dttrxntime
					  at time zone 'utc' at time zone timeZone <= enddate and payeevpa = vpaaddress);
...
AS $BODY$
BEGIN
Return query select vcmsgid as "Unique ID", dtTrxnTime at time zone 'utc' at time zone timeZone as "Time",
		 payervpa,
		payeevpa,
		dTransAmount as "Amount", ld.vcremark as "Remark", ld.dscore as  "Rule Score" from transactions.vw_livetrans_with_vpa_and_account_from_joins L,
		transactions.LiveDecisionDetails ld where
		ld.ilivemessageid= L.ilivemessageid and L.score= RiskScore and ld.dscore > 0 and dtTrxnTime >now() - lastTime::interval
...
Elseif(vpatype='Payee') Then
		 Return query select L.iLiveMessageID as "ID", vcmsgid  as "Unique ID", L.dtTrxnTime at time zone 'utc' at time zone timezone as "Time",
                             ...
                             vcpayeraccount,
                             vcpayeeaccount
                      from transactions.vw_livetrans_with_vpa_and_account_from_joins L where payeevpa = useraddress and (CAST(dtTrxnTime AS date) = CURRENT_DATE-1 or CAST(dtTrxnTime AS date) = CURRENT_DATE) limit maxrecord;
...
If(txntype='previous') Then
	If(vpatype='Payee') Then
		   Return query select L.iLiveMessageID as "ID", L.vcmsgid  as "Unique ID", L.dtTrxnTime at time zone 'utc' at time zone timezone as "Time",
                               ...
                               L.vcrulename as "Rule" from transactions.vw_livetrans_with_vpa_and_account_from_joins L where L.iLiveMessageID < ilivemsgid
...
BEGIN
	...
	If(vpatype='Payee') Then
		   Return query select L.iLiveMessageID as "ID", L.vcmsgid  as "Unique ID", L.dtTrxnTime at time zone 'utc' at time zone timezone as "Time",
                               ...
                               payeevpa,
                               payeename,
                               L.dTransAmount as "Amount", L.bFRMPassed as "FRM Pass", L.score as "Score", vcrulename as "Rule" from
