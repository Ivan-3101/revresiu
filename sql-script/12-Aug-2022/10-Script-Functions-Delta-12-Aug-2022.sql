 ...
 If(party='Account') Then
			  If(vpatype='Payee') Then
				num =(select count(vld.ilivemessageid) from transactions.vw_livetrans_dashboard_with_joins vld where
					  cast(vld.dttrxntime at time zone 'utc' at time zone timeZone as date)= dateinput and vcpayeeaccount = vpaaddress);
...
End If;
	...
	Elseif(vpatype='Payer') Then
	 ...
	 num =(select count(vld.ilivemessageid) from transactions.vw_livetrans_dashboard_with_joins vld where   cast(vld.dttrxntime at time zone 'utc' at time zone timeZone as date)= dateinput
	...
	Return query SELECT vld.ilivemessageid, vld.vcmsgid, vld.vcuniquetransid, vld.dttrxntime at time zone 'utc' at time zone timeZone,
                        ...
                        payervpa,
                        payeevpa
                 FROM transactions.vw_livetrans_dashboard_with_joins vld where  cast(vld.dttrxntime at time zone 'utc' at time zone timeZone as date)= dateinput
                                                                           and vcpayeraccount = vpaaddress;
...
Elseif(vpatype='Both') Then
 ...
 num =(select count(vld.ilivemessageid)  FROM transactions.vw_livetrans_dashboard_with_joins vld where
	    cast(vld.dttrxntime at time zone 'utc' at time zone timeZone as date)= dateinput and
...
elseif(party='VPA') then
  If(vpatype='Payee') Then
				num =(select count(vld.ilivemessageid) from transactions.vw_livetrans_dashboard_with_joins vld where
					   cast(vld.dttrxntime at time zone 'utc' at time zone timeZone as date)= dateinput and payeevpa = vpaaddress);
...
End If;
	...
	Elseif(vpatype='Payer') Then
	 ...
	 num =(select count(vld.ilivemessageid) from transactions.vw_livetrans_dashboard_with_joins vld where
		   cast(vld.dttrxntime at time zone 'utc' at time zone timeZone as date)= dateinput and payervpa = vpaaddress);
	 if(maxrecord > num )Then
	Return query SELECT vld.ilivemessageid, vld.vcmsgid, vld.vcuniquetransid, vld.dttrxntime at time zone 'utc' at time zone timeZone,
                        vld.dsettledamount, vld.dtransamount, vld.dfailedamount, vld.bnewpayer, vld.bnewpayeeforpayer, vld.bfrmpassed, vld.dtupdatedtime, vld.observations::text,
                                          vld.txnclass, vld.mcc, vld.device, vld.ip, vld.location, vld.deviceid, vld.os, vld.vcrulename, vld.score, vld.status, vld.decisiondetails,
                        payervpa,
                        payeevpa
                 FROM transactions.vw_livetrans_dashboard_with_joins vld where  cast(vld.dttrxntime at time zone 'utc' at time zone timeZone as date)= dateinput and payervpa = vpaaddress;
