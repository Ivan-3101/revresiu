...
BEGIN
			  If(vpatype='Payee') Then
				num =(select count(vld.ilivemessageid) from transactions.vw_livetrans_dashboard vld, masters.vpa where ipayeevpaid=ivpaid and vcaddress = vpaaddress);
...
End If;
	...
	Elseif(vpatype='Payer') Then
	 ...
	 num =(select count(vld.ilivemessageid) from transactions.vw_livetrans_dashboard vld, masters.vpa where ipayervpaid=ivpaid and vcaddress = vpaaddress);
...
Elseif(vpatype='Both') Then
 ...
 num =(select count(vld.ilivemessageid)  FROM transactions.vw_livetrans_dashboard vld, masters.vpa payervpa, masters.vpa payeevpa where vld.dttrxntime>=startdate
         and vld.dttrxntime<=enddate and payeevpa.ivpaid=vld.ipayeevpaid and payervpa.ivpaid=vld.ipayervpaid
