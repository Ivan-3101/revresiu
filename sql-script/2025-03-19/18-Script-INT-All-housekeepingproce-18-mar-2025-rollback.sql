...
LANGUAGE 'plpgsql'
AS $BODY$
DECLARE MAX_BDD_ID BIGINT;
BEGIN
	SELECT COALESCE(MAX(bdd_id), 0) INTO MAX_BDD_ID FROM analytics.batch_rule_triggered WHERE itenantid = itenantid_value;
-----------
    ...
    with tbt as (
	 SELECT 
		...
		where tbt.btrans_pk > MAX_BDD_ID AND tbt.dttrxntime::date < CDATE AND itenantid = itenantid_value
	),
	tt as (
		select tbt.idecisionid, tbt.iruleid, tbt.vcmsgid,tbt.dttrxntime,
        tbt.ivpaid, tbt.vcaddr, tbt.iaccountid, tbt.vcaccountexternalid,
        tbt.icustomerid, tbt.vccustomerexternalid,
        case 
			when tbt.dscore ~ '^[0-9]+(\.[0-9]+)?$'
			then tbt.dscore::numeric::int
			else 0
		end as dscore, 
	    tbt.txn_score, tbt.class_name,
		tbt.btrans_pk, tbt.itenantid,
		d.vcdecisionname, r.vcrulename
		from tbt LEFT JOIN masters.rules r ON r.iruleid = tbt.iruleid AND r.itenantid = tbt.itenantid
       LEFT JOIN masters.decisions d ON d.idecisionid = r.idecisionid AND d.itenantid = r.itenantid
	)
	select tt.idecisionid, tt.iruleid, tt.vcmsgid,tt.dttrxntime,
...
DECLARE 
    ...
    cnt_null_vcclassname INT := 0;
BEGIN
    SELECT COALESCE(MAX(ilivemessageid), 0) INTO MAX_DD_ID FROM analytics.rule_triggered WHERE itenantid = itenantid_value;
    ...
    WITH trans AS (
        SELECT 
            ...
            tt.itenantid,
            tt.score as txn_score, tt.vcclassname,
            ((jsonb_array_elements_text(tt.result->'score'->'decisiondetails')::json->>'ruleid')::numeric)::int as iruleid,
        ...
        WHERE tt.ilivemessageid > MAX_DD_ID 
          ...
          AND tt.vcclassname IS NOT NULL
    ), 
    tt AS (
        SELECT 
    ...
    -- Counting records with vcclassname is null
    SELECT COUNT(*) INTO cnt_null_vcclassname 
    FROM analytics.trans tt 
    WHERE tt.ilivemessageid > MAX_DD_ID 
      AND tt.itenantid = itenantid_value 
