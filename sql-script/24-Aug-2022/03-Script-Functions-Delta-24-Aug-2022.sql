...
DROP FUNCTION IF EXISTS ui.gettxnprofileselectedtxn_new(character varying, character varying, character varying, character varying, character varying, date);
...
if(party = 'VPA')then
	If(vpatype='Payee') Then
		   Return query select L.iLiveMessageID as "ID", L.vcmsgid  as "Unique ID", L.dtTrxnTime at time zone 'Asia/Kolkata' at time zone timezone as "Time",
                        ...
                        where vcmsgid=msgid and payeevpa = vpaaddress;
...
Elseif(vpatype='Payer') Then
		 Return query select L.iLiveMessageID as "ID", L.vcmsgid  as "Unique ID", L.dtTrxnTime at time zone 'Asia/Kolkata' at time zone timezone as "Time",
                             ...
                             L.dTransAmount as "Amount", L.bFRMPassed as "FRM Pass", L.score as "Score", vcrulename as "Rule", L.vcpayeraccount, L.vcpayeeaccount  from
                                transactions.vw_livetrans_with_vpa_and_account_from_joins L where vcmsgid = msgid and payervpa = vpaaddress;
Elseif(vpatype='Both') Then
		 Return query select L.iLiveMessageID as "ID", L.vcmsgid  as "Unique ID", L.dtTrxnTime at time zone 'Asia/Kolkata' at time zone timezone as "Time",
                             ...
                             payeename,
                             L.dTransAmount as "Amount", L.bFRMPassed as "FRM Pass", L.score as "Score", vcrulename as "Rule", L.vcpayeraccount, L.vcpayeeaccount  from
                                transactions.vw_livetrans_with_vpa_and_account_from_joins L where vcmsgid = msgid and (payervpa = vpaaddress or payeevpa = vpaaddress) ;
...
elseif(party = 'Account' )then
	If(vpatype='Payee') Then
		   Return query select L.iLiveMessageID as "ID", L.vcmsgid  as "Unique ID", L.dtTrxnTime at time zone 'Asia/Kolkata' at time zone timezone as "Time",
                        ...
                        where vcmsgid=msgid and vcpayeeaccount = vpaaddress;
...
Elseif(vpatype='Payer') Then
		 Return query select L.iLiveMessageID as "ID", L.vcmsgid  as "Unique ID", L.dtTrxnTime at time zone 'Asia/Kolkata' at time zone timezone as "Time",
                             ...
                             payeename,
                             L.dTransAmount as "Amount", L.bFRMPassed as "FRM Pass", L.score as "Score", vcrulename as "Rule", L.vcpayeraccount, L.vcpayeeaccount  from
                                transactions.vw_livetrans_with_vpa_and_account_from_joins L where vcmsgid = msgid and vcpayeraccount = vpaaddress;
...
Elseif(vpatype='Both') Then
		 Return query select L.iLiveMessageID as "ID", L.vcmsgid  as "Unique ID", L.dtTrxnTime at time zone 'Asia/Kolkata' at time zone timezone as "Time",
                             ...
                             payeename,
                             L.dTransAmount as "Amount", L.bFRMPassed as "FRM Pass", L.score as "Score", vcrulename as "Rule", L.vcpayeraccount, L.vcpayeeaccount  from
                                transactions.vw_livetrans_with_vpa_and_account_from_joins L where vcmsgid = msgid and (vcpayeraccount = vpaaddress or vcpayeeaccount = vpaaddress);
