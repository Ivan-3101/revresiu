...
SELECT  vcemailid, count(vcemailid)	FROM ui.webuser where istatus = 1 group by vcemailid;
...
UPDATE ui.webuser
	SET vcusername = case when  vcemailid is not null
	then vcemailid
	else vcusername
end;


UPDATE ui.webuseraudit
	SET vcusername = case when  vcemailid is not null
	then vcemailid
	else vcusername
end;
