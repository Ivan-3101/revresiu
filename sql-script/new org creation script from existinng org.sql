...
DECLARE
    ...
    old_itenantid_value INT = 14;
    itenantid_value INT := 28;
    vctenantname_value VARCHAR := 'MK';
    vcorgname_value VARCHAR := 'MK';
    iorg_value INT := 10;
    new_org_id INT := 13;
    ...
    api_key_expiry_date VARCHAR := '2025-11-25';
    -- tenant_value : =
...
/* ------------------------------- Inserting into rolemenuaccessmap - ------------------------------- */
  ...
  INSERT INTO ui.webusermapping(
    ...
    INSERT INTO ui.dashboardresultset(idashboardresultsetid, iresultsetorder, vcdashboardresultsetcolumnjson, vcdashboardresultsetlayout, vcdashboardresultsetname, idashboardqueryid, idashboardid, vcdashboardresultsetschema, icolsize, irowno, dtlastupdatedtimestamp, iuserid, itenantid, iorgid)
        SELECT idashboardresultsetid, iresultsetorder, vcdashboardresultsetcolumnjson, vcdashboardresultsetlayout, vcdashboardresultsetname, idashboardqueryid, idashboardid, vcdashboardresultsetschema, icolsize, irowno, dtlastupdatedtimestamp, iuserid, itenantid_value, new_org_id
        ...
        and vcgroupid not in (select distinct group_id_ from camunda.act_ru_authorization where group_id_ IS NOT NULL);
         ...
         --insert cadmin user entries
        INSERT INTO camunda.act_id_user (id_, rev_, first_, last_, email_, pwd_, salt_)
        SELECT wb.iuserid::varchar,  1, wb.vcfirstname, wb.vclastname, wb.vcemailid, '{SHA-512}Hv0v7GCpl0i43FmTGn+IJ6V8nFUvh3RNQXQ4LDomrlCeFuVPxcxbstRUgmamaWSTQFtQ6V0rS+2K0CaauNmTCA==',
        'YxsoNMYlWRfYnoxWi0zG3Q==' from ui.webuser wb where vcemailid  in (cadmin_email, madmin_email) and iorgid = new_org_id;
        --for user auth
        INSERT INTO camunda.act_ru_authorization(id_, rev_, type_, user_id_, resource_type_, resource_id_, perms_)
        select ENCODE(gen_random_bytes(32), 'hex'), 1, 1, wb.iuserid::varchar , 1, '*',  2147483647
        from ui.webuser wb where vcemailid in (cadmin_email, madmin_email) and iorgid = new_org_id;
        --map to usermgmt group
        INSERT INTO camunda.act_id_membership(user_id_, group_id_)
        SELECT wb.iuserid::varchar , 'usermgmt' from ui.webuser wb where vcemailid in (cadmin_email, madmin_email) and iorgid = new_org_id;
