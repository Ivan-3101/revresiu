...
analytics_ids AS (
    ...
    WHERE sr.dtentrydatetime >= CAST(:StartDate AS DATE) 
      AND sr.dtentrydatetime < CAST(:EndDate AS DATE)
      AND sr.itenantid = :tenantid
      AND sr.vcrequestid IS NOT NULL
),
optimized_transaction_scorerequests AS (
    SELECT 
        DATE(tsr.dtentrydatetime) AS date_val,
        JSON_EXTRACT_SCALAR(tsr.vcrequestdata, ''$.txn.class'') AS class_val,
        COUNT(CASE WHEN tsr.vcrequestid IS NULL THEN 1 END) AS failed_count,
        COUNT(CASE WHEN tsr.vcrequestid IS NOT NULL THEN 1 END) AS success_count
    FROM postgresql.transactions.scorerequests tsr
    LEFT JOIN analytics_ids ai 
        ON tsr.vcrequestid = ai.vcrequestid 
        AND tsr.itenantid = ai.itenantid
    WHERE tsr.dtentrydatetime >= CAST(:StartDate AS DATE)
      AND tsr.dtentrydatetime < CAST(:EndDate AS DATE)
      AND tsr.itenantid = :tenantid
      AND ai.vcrequestid IS NULL
    GROUP BY 
        DATE(tsr.dtentrydatetime),
        JSON_EXTRACT_SCALAR(tsr.vcrequestdata, ''$.txn.class'')
)
SELECT 
    date_val AS "Date",
    class_val AS "Class",
